function GradientAscentEgComplex()
...
GridPoints=[-3:0.25:7];
    for i = 1:length(GridPoints)
        for j = 1:length(GridPoints)
            ...
            [Iters(i,j), height(i,j)] =GradAscent(StartPt, NumSteps, LRate);
        end
    end 


pcolor(GridPoints, GridPoints, height);
colorbar;
...
function[Iters, height] = GradAscent(StartPt,NumSteps,LRate)
%PauseFlag=1;
hold on;
Iters = 0;
...
for i = 1:NumSteps
    ...
    height = ComplexLandscape(StartPt(1),StartPt(2));
    ...
    % Uncommented to remove bounds checking 
...
function[Iters, height] = HillClimb(StartPt,NumSteps,MaxMutate)
%PauseFlag=1;
hold on;
Iters = 0;
...
for i = 1:NumSteps
    ...
    %Commented to remove the bounds checking
    %NewPt = max([NewPt;-2 -2]);
    %NewPt = min([NewPt;2 2]);
    ...
    newHeight = ComplexLandscape(NewPt(1),NewPt(2));
